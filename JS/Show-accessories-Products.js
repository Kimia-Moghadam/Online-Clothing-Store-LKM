...
.then(data => {
  const container = document.getElementById('Show-accessories-Products');
  
  data.slice(5,8).forEach(product => {
    let shortTitle = product.title.length > 10 ? product.title.substring(0, 40) + '' : product.title;
    debugger
    const productCard = `
